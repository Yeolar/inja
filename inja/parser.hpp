...
class Parser {
  ...
  acc::StringPiece json_literal_start;
  ...
  void add_json_literal(const char* content_ptr) {
    acc::StringPiece json_text(json_literal_start.data(), tok.text.data() - json_literal_start.data() + tok.text.size());
    current_expression_list->rpn_output.emplace_back(std::make_shared<LiteralNode>(json::parse(json_text.str()), json_text.data() - content_ptr));
  }
    ...
    while (tok.kind != closing && tok.kind != Token::Kind::Eof) {
      ...
      case Token::Kind::Id: {
        ...
        // Functions
          ...
          operator_stack.emplace(std::make_shared<FunctionNode>(tok.text.str(), tok.text.data() - tmpl.content.c_str()));
        ...
        // Variables
          ...
          current_expression_list->rpn_output.emplace_back(std::make_shared<JsonNode>(tok.text.str(), tok.text.data() - tmpl.content.c_str()));
        }
  ...
  bool parse_statement(Template &tmpl, Token::Kind closing, acc::StringPiece path) {
    ...
    if (tok.text == "if") {
      ...
      if (tok.kind != Token::Kind::Id || tok.text != "in") {
      ...
      json json_name = json::parse(tok.text.str());
  ...
  void parse_into(Template &tmpl, acc::StringPiece path) {
...
public:
  ...
  Template parse(acc::StringPiece input, acc::StringPiece path) {
    auto result = Template(input.str());
    parse_into(result, path);
    return result;
  }

  Template parse(acc::StringPiece input) {
    return parse(input, "./");
  }

  void parse_into_template(Template& tmpl, acc::StringPiece filename) {
    auto find_last_of = [](acc::StringPiece sp) -> size_t {
      for (size_t i = sp.size(); i-- > 0; )
        if (sp[i] == '/' || sp[i] == '\\')
          return i;
      return acc::StringPiece::npos;
    };
    acc::StringPiece path = filename.subpiece(0, find_last_of(filename) + 1);
  ...
  std::string load_file(acc::StringPiece filename) {
    std::ifstream file;
    open_file_or_throw(filename.str(), file);
